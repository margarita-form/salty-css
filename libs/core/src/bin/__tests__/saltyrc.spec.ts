...
describe('upsertProjectInRc', () => {
  it('creates a fresh saltyrc when none exists', () => {
    const { content, changed, created } = upsertProjectInRc(undefined, 'apps/web', 'react');
  ...
  it('appends a new project entry when missing', () => {
    ...
    const { content, changed, created } = upsertProjectInRc(existing, 'apps/site', 'astro');
    expect(created).toBe(false);
    expect(changed).toBe(true);
    const parsed = JSON.parse(content);
    expect(parsed.projects).toHaveLength(2);
    expect(parsed.projects[1]).toEqual({ dir: 'apps/site', framework: 'astro' });
  });

  it('is a no-op when the project already exists (preserves existing framework)', () => {
    const existing = JSON.stringify(
      { defaultProject: 'apps/web', projects: [{ dir: 'apps/web', framework: 'react' }] },
      null,
      2
    );
    const { content, changed } = upsertProjectInRc(existing, 'apps/web', 'astro');
