...
export class SaltyCompiler {
  ...
  constructor(
    ...
    private options: SaltyCompilerOptions = {},
  ) {
  ...
  public generateCss = async (clean = true) => {
    try {
      ...
      const versionNumber = __VERSION__ || '0.0.0';
    ...
    await Promise.all(
      [...configFiles].map(async (src) => {
        ...
        Object.entries(contents).forEach(([name, value]) => {
          ...
          else if (value.isDefineImport) generationResults.imports.push((value as any)._setPath(src));
          else if (value.isDefineFont) generationResults.fonts.push(value as any);
        });
      }),
    );
    ...
    for (const factory of generationResults.imports) {
      ...
      for (const spec of factory._current) {
        try {
          ...
          logger.error(`Failed to resolve defineImport(${JSON.stringify(url)}) from ${sourceFile}: ${(e as Error).message}`);
        }
      }
    }
